...
function MessageItem({ message }: { message: Message }) {
  ...
  useEffect(() => {
    if (isAssistant) {
      ...
      // Remove risk score and extract it
      const riskMatch = content.match(/\[RISK:\s*(\d+)\]/i) || content.match(/Risk\s*Skoru:\s*(\d+)/i);
      if (riskMatch) {
        setRiskScore(parseInt(riskMatch[1]));
        content = content.replace(/\[RISK:\s*\d+\]/i, '').replace(/Risk\s*Skoru:\s*\d+/i, '').trim();
      }
      ...
      if (content.startsWith('{') && content.endsWith('}')) {
        try {
          ...
          content = parsed.text || parsed.content || parsed.analysis || parsed.report || parsed.message || content;
        } catch (e) {
      ...
      content = content.replace(/^(text|content|message|report|analysis):\s*/i, '');
      
      // Normalize newlines and remove literal \n strings if they appear as text
      content = content.replace(/\\n/g, '\n');
      
      // Final polish: remove any trailing/leading whitespace and normalize paragraph breaks
      setCleanContent(content.trim().replace(/\n{3,}/g, '\n\n'));
    }
  }, [message, isAssistant]);
